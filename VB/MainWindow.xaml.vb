...
Namespace DXSample
	...
	Public Class VisibleListDropTarget
		...
		Friend Function GetColumnByDragElement(ByVal element As UIElement) As ColumnBase
			Dim columnList As FrameworkElement = TryCast(element, ColumnList)
			If columnList IsNot Nothing Then
				Dim view As DataViewBase = (CType(columnList.DataContext, CustomColumnChooserContent)).View
				Return (TryCast(view.DataControl, GridControl)).Columns((CType(Me, IDropTarget)).Index)
			Else
				Return TryCast((TryCast(element, GridColumnHeader)).DataContext, GridColumn)
			End If
		End Function
		...
		Private Property Index() As Integer Implements IDropTarget.Index
			Get
				Return privateIndex
			End Get
